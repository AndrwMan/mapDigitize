...
if image is None:
    ...
    cv2.imwrite('./imgs/digitized/edges_processed8.jpg', edges)  # Save the processed edges image for review
    ...
    if largest_contour is not None and len(largest_contour) > 0:
        ...
        cv2.imwrite('./imgs/digitized/largest_contour_filtered8.jpg', contour_image)  # Save the largest contour image for review
        ...
        cv2.imwrite('./imgs/digitized/map_isolated8.jpg', map_isolated)  # Save the isolated map for review

        # Get the bounding box of the largest contour
        x, y, w, h = cv2.boundingRect(largest_contour)
        
        # Crop the isolated map to the bounding box
        cropped_map = map_isolated[y:y+h, x:x+w]
        cv2.imwrite('./imgs/digitized/cropped_map.jpg', cropped_map)  # Save the cropped map image for review

        print("Map area isolated, cropped, and saved.")
    else:
